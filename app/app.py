...
#             players.append(f"{player['player_name']} ({player['player_id']})")
...
#             summary = get_match_summary(st.session_state.selected_match_id)
#             if summary:
#                 st.subheader("Resumo da Partida")
#                 st.write(summary)
#             else:
#                 st.error("Não foi possível gerar o resumo da partida.")
    
#     with col2:
#         selected_player = st.selectbox("Selecione o jogador:", st.session_state.get('players', []))
        
#         if st.button("Gerar Perfil do Jogador"):
#             player_id = selected_player.split('(')[-1].split(')')[0]
#             profile = get_player_profile(st.session_state.selected_match_id, player_id)
#             if profile:
#                 st.subheader("Perfil do Jogador")
#                 st.write(profile)
#             else:
#                 st.error("Não foi possível gerar o perfil do jogador.")
